...
class ReservationHelper{
    ...
    {
        $tableIds = Table::where('status', 1)->where('floor_id', $floor)->where('capacity_of_person', intval($person))->pluck('id');
        if(!count($tableIds)) {
            $nexttable = Table::where('status', 1)->where('floor_id', $floor)->orderBy('capacity_of_person','ASC')->where('capacity_of_person', '>' , intval($person))->first();
            $tableIds = Table::where('status', 1)->where('floor_id', $floor)->where('capacity_of_person', intval(@$nexttable->capacity_of_person))->pluck('id');
        }
